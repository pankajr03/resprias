...
class RsformControllerComponents extends RsformController
{
	...
	public function save()
	{
		...
		else
		{
			...
			$value = $original != $published ? $published : null;
			$component = (object) array(
		...
		$app->triggerEvent('onRsformComponentSave', array('id' => $componentIdToEdit, 'formId' => $formId, 'isNew' => $just_added, 'type' => $componentType, 'value' => $value));
    ...
    public function changeStatus()
	{
		...
		$formId = $app->input->getInt('formId');
		$task = $this->getTask();
		$msg = $task == 'publish' ? 'RSFP_ITEMS_PUBLISHED' : 'RSFP_ITEMS_UNPUBLISHED';

		$model->componentsChangeStatus($task == 'publish' ? 1 : 0);
		$componentId = $model->getComponentId();

		$app->triggerEvent('onRsformComponentChangeStatus', array('ids' => $componentId, 'formId' => $formId, 'value' => $task == 'publish' ? 1 : 0));
		...
		elseif (!$ajax)
		{
			$app->input->set('view', 'formajax');
			$app->input->set('layout', 'component_published');
	...
	public function remove()
	{
		...
		if ($cids) {
			...
			$names = $db->setQuery($query)->loadObjectList('ComponentId');
			$query->clear()
		...
		$app->triggerEvent('onRsformBackendAfterComponentDeleted', array($componentIds, $formId, $names));
