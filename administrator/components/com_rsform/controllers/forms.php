	...
	public function changeStatus()
	{
		...
		if ($total > 0)
		{
			$query = $db->getQuery(true)
				...
				->set($db->qn('Published') . ' = ' . $db->q($value))
				->where($db->qn('FormId') . ' IN (' . implode(',', $db->q($cid)) . ')');
			$db->setQuery($query)->execute();
		}

		Factory::getApplication()->triggerEvent('onRsformFormChangeStatus', array('formIds' => $cid, 'value' => $value));
