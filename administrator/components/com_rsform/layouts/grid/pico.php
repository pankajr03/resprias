...
class RSFormProGridPico extends RSFormProGrid
{
		...
		foreach ($this->pages as $page_index => $rows)
		{
			...
			foreach ($rows as $row_index => $row)
			{
				...
				foreach ($row['columns'] as $column_index => $fields)
				{
					...
					$html[] = "\t"."\t".'<div class="pico-col-' . (int) $size . '">';
	...
	protected function generateField($data)
	{
		...
		else
		{
			...
			$html[] = "\t"."\t"."\t".'<div class="' . ($this->formOptions->FormLayoutFlow == static::FLOW_HORIZONTAL ? 'pico-row-fluid' : 'pico-margin') . ' rsform-block rsform-block-' . $block . ' rsform-type-' . $type . $placeholders['error'] . '">';
			...
			if ($this->formOptions->FormLayoutFlow == static::FLOW_VERTICAL)
			{
				...
				$labelTag = $this->isCheckable($data) || $this->isButton($data) || $this->isMissingLabel($data) ? 'span' : 'label';
				$label .= "\t"."\t"."\t"."\t".'<' . $labelTag . ' class="formControlLabel"';
				$label .= $this->generateFor($data);
				$label .= '{if ' . $placeholders['description'] . '}';
				$label .= ' data-tooltip="' . $placeholders['description'] . '"';
				$label .= '{/if}';
				$label .= '>';
			...
			{
				$label = '<div class="pico-col-3">' . $label . '</div>';
			}
				$html[] = $label;
	...
	protected function fieldAttributes($data)
	{
		...
		{
			$attr .= ' aria-labelledby="' . $data->ComponentName . '-grouplbl"';
		}
