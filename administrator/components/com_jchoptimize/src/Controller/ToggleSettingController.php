...
class ToggleSettingController extends BaseController implements MVCFactoryDecoratorAwareInterface
{
    ...
    public function display($cachable = false, $urlparams = []): void
    {
        ...
        $model = $this->getModel('Configure', 'Administrator');

        try {
            $model->toggleSetting($setting);
        } catch (ExceptionInterface) {
        }

        /** @var JsonView $view */
        $view = $this->getView('ToggleSetting', 'json', 'Administrator');
        $view->setModel($model);
