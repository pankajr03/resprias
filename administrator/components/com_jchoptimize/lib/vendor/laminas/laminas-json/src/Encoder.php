...
class Encoder
{
        ...
        if ($this->cycleCheck) {
            if ($this->wasVisited($value)) {
                if (!isset($this->options['silenceCyclicalExceptions']) || $this->options['silenceCyclicalExceptions'] !== \true) {
                    throw new RecursionException(sprintf('Cycles not supported in JSON encoding; cycle introduced by class "%s"', $value::class));
                }
    ...
    protected static function utf82utf16($utf8)
    {
        ...
        return match (strlen($utf8)) {
            ...
            // see: http://www.cl.cam.ac.uk/~mgk25/unicode.html#utf-8
            2 => chr(0x7 & ord($utf8[0]) >> 2) . chr(0xc0 & ord($utf8[0]) << 6 | 0x3f & ord($utf8[1])),
            // Return a UTF-16 character from a 3-byte UTF-8 char;
            // see: http://www.cl.cam.ac.uk/~mgk25/unicode.html#utf-8
            3 => chr(0xf0 & ord($utf8[0]) << 4 | 0xf & ord($utf8[1]) >> 2) . chr(0xc0 & ord($utf8[1]) << 6 | 0x7f & ord($utf8[2])),
