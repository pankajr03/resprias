...
namespace _JchOptimizeVendor\V91\Spatie\Robots;
...
class RobotsHeaders
{
    ...
    {
        return $this->robotHeadersProperties[$userAgent]['none'] ?? $this->robotHeadersProperties['*']['none'] ?? \false;
    }
    protected function parseHeaders(array $headers): array
    {
        ...
        return array_reduce($robotHeaders, function (array $parsedHeaders, $header) {
            ...
            $userAgent = count($headerParts) === 3 ? trim($headerParts[1]) : '*';
            $options = end($headerParts);
            $parsedHeaders[$userAgent] = ['noindex' => strpos(strtolower($options), 'noindex') !== \false, 'nofollow' => strpos(strtolower($options), 'nofollow') !== \false, 'none' => strpos(strtolower($options), 'none') !== \false];
    ...
    protected function filterRobotHeaders(array $headers): array
    {
        return array_filter($headers, function ($header) use ($headers) {
            ...
            return strpos(strtolower($header), 'x-robots-tag') === 0 || strpos(strtolower($headerContent), 'x-robots-tag') === 0;
