...
use _JchOptimizeVendor\V91\Spatie\Browsershot\Browsershot;
use _JchOptimizeVendor\V91\Spatie\Crawler\CrawlObservers\CrawlObserver;
use _JchOptimizeVendor\V91\Spatie\Crawler\CrawlObservers\CrawlObserverCollection;
use _JchOptimizeVendor\V91\Spatie\Crawler\CrawlProfiles\CrawlAllUrls;
use _JchOptimizeVendor\V91\Spatie\Crawler\CrawlProfiles\CrawlProfile;
use _JchOptimizeVendor\V91\Spatie\Crawler\CrawlQueues\ArrayCrawlQueue;
use _JchOptimizeVendor\V91\Spatie\Crawler\CrawlQueues\CrawlQueue;
use _JchOptimizeVendor\V91\Spatie\Crawler\Exceptions\InvalidCrawlRequestHandler;
use _JchOptimizeVendor\V91\Spatie\Crawler\Handlers\CrawlRequestFailed;
use _JchOptimizeVendor\V91\Spatie\Crawler\Handlers\CrawlRequestFulfilled;
use _JchOptimizeVendor\V91\Spatie\Robots\RobotsTxt;
use _JchOptimizeVendor\V91\Tree\Node\Node;
...
class Crawler
{
    ...
    protected bool $respectRobots = \true;
    protected bool $rejectNofollowLinks = \true;
    ...
    protected static array $defaultClientOptions = [RequestOptions::COOKIES => \true, RequestOptions::CONNECT_TIMEOUT => 10, RequestOptions::TIMEOUT => 10, RequestOptions::ALLOW_REDIRECTS => \false, RequestOptions::HEADERS => ['User-Agent' => self::DEFAULT_USER_AGENT]];
    ...
    public function __construct(protected Client $client, protected int $concurrency = 10)
    {
    ...
    public function startCrawling(UriInterface|string $baseUrl)
    {
        ...
        if ($this->robotsTxt->allows((string) $crawlUrl->url, $this->getUserAgent()) || !$this->respectRobots) {
    ...
    {
        while ($this->reachedCrawlLimits() === \false && $this->crawlQueue->hasPendingUrls()) {
            $pool = new Pool($this->client, $this->getCrawlRequests(), ['concurrency' => $this->concurrency, 'options' => $this->client->getConfig(), 'fulfilled' => new $this->crawlRequestFulfilledClass($this), 'rejected' => new $this->crawlRequestFailedClass($this)]);
    ...
    {
        while ($this->reachedCrawlLimits() === \false && $crawlUrl = $this->crawlQueue->getPendingUrl()) {
            if ($this->crawlProfile->shouldCrawl($crawlUrl->url) === \false || $this->crawlQueue->hasAlreadyBeenProcessed($crawlUrl)) {
