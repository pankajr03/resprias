...
use _JchOptimizeVendor\V91\Symfony\Component\DomCrawler\Crawler as DomCrawler;
use _JchOptimizeVendor\V91\Symfony\Component\DomCrawler\Link;
use _JchOptimizeVendor\V91\Tree\Node\Node;

use function _JchOptimizeVendor\V91\collect;
...
class LinkAdder
{
    ...
    public function addFromHtml(string $html, UriInterface $foundOnUrl): void
    {
        ...
        collect($allLinks)->filter(fn(UriInterface $url) => $this->hasCrawlableScheme($url))->map(fn(UriInterface $url) => $this->normalizeUrl($url))->filter(function (UriInterface $url) use ($foundOnUrl) {
            if (!$node = $this->crawler->addToDepthTree($url, $foundOnUrl)) {
                return \false;
            }
            return $this->shouldCrawl($node);
        })->filter(fn(UriInterface $url) => !str_contains($url->getPath(), '/tel:'))->each(function (UriInterface $url) use ($foundOnUrl) {
