...
class MapmarkersModel extends ListModel
{
    ...
    protected function getListQuery() 
    {
        ...
        $query->select('h.id,h.title,h.mapid,h.published,h.publish_up,h.publish_down,h.catid,h.icontype,h.ordering,h.rating_value,h.createdbyuser,h.access,h.userorder,'.
        'c.title as category, c.language as category_language,m.title as mapname,g.title as markergroupname, usr.username, usr.name fullusername,'.
			...
			$subQuery = $db->getQuery(true)
				...
				->where(
					[
						...
						$db->quoteName('type_alias') . ' = ' . $db->quote('com_zhgooglemap.mapmarker'),
					]
				);
			...
			$query->join(
				...
				->where(
					[
						...
						$db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_zhgooglemap.mapmarker'),
					]
				)
		...
		{
			$access = (int) $access;
			$query->where($db->quoteName('h.access') . ' = :access')
				->bind(':access', $access, ParameterType::INTEGER);
		}
		elseif (is_array($access))
		{
			$access = ArrayHelper::toInteger($access);
			$query->whereIn($db->quoteName('h.access'), $access);
		}
