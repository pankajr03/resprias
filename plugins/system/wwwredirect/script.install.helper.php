...
class PlgSystemWwwredirectInstallerScriptHelper
{
	public $name            = '';
	public $alias           = '';
	public $extname         = '';
		...
		if ($show_message)
		{
			Factory::getApplication()->enqueueMessage(
				\Joomla\CMS\Language\Text::sprintf(
					...
					\Joomla\CMS\Language\Text::_('COM_INSTALLER_TYPE_TYPE_' . strtoupper($type))
				)
			);
		}
	}
	...
	public function addInstalledMessage()
	{
		Factory::getApplication()->enqueueMessage(
			\Joomla\CMS\Language\Text::sprintf(
				\Joomla\CMS\Language\Text::_($this->install_type == 'update' ? 'W357_THE_EXTENSION_HAS_BEEN_UPDATED_SUCCESSFULLY' : 'W357_THE_EXTENSION_HAS_BEEN_INSTALLED_SUCCESSFULLY'),
				'<strong>' . \Joomla\CMS\Language\Text::_($this->name) . '</strong>',
	...
	public function getPrefix()
	{
		...
		{
			case 'plugin';
				return \Joomla\CMS\Language\Text::_('plg_' . strtolower($this->plugin_folder));

			case 'component':
				return \Joomla\CMS\Language\Text::_('com');

			case 'module':
				return \Joomla\CMS\Language\Text::_('mod');

			case 'library':
				return \Joomla\CMS\Language\Text::_('lib');
	...
	{
		return \Joomla\CMS\Language\Text::_('W357_' . strtoupper($this->getPrefix()));
	}
	...
	public function canInstall()
	{
		...
		Factory::getApplication()->enqueueMessage(\Joomla\CMS\Language\Text::_('W357_ERROR_PRO_TO_FREE'), 'error');
		...
		Factory::getApplication()->enqueueMessage(
			html_entity_decode(
				\Joomla\CMS\Language\Text::sprintf(
					...
					'</a>',
					\Joomla\CMS\Language\Text::_($this->name)
				)
			), 'error'
		);
