...
function modChrome_T3Section($module, &$params, &$attribs) { 
	...
	if($params->get('module-width')) {
		$moduleWidth    = 'full-width';
	}

	if($params->get('mask-box')) {
		$maskModule    = $params->get('mask-box');
	}

	if (!empty ($module->content)) {
		$html = "<{$moduleTag} class=\"t3-section module{$moduleClassSfx} {$moduleClass} mask-box-{$maskModule}\" id=\"Section{$module->id}\">" .
					"<div class=\"{$moduleWidth}\"><div class=\"module-inner\">";
